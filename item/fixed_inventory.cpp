...
namespace
{
    ...
    auto lower_helper(C && c, chimera::ConstPointer<chimera::Item> const & item)
    {
        return std::lower_bound(
            ...
            [](auto const & it, auto id) { return it->id() < id; });
    }
    ...
    auto upper_helper(C && c, chimera::ConstPointer<chimera::Item> const & item)
    {
        return std::upper_bound(
            std::begin(c), std::end(c), item->id(),
            [](auto id, auto const & it) { return id < it->id(); });
    }
...
auto FixedSizeInventory::count(ConstPointer<Item> const & item) const noexcept
    -> std::size_t
{
...
auto FixedSizeInventory::contains(ConstPointer<Item> const & item) const
    noexcept -> bool
{
    auto const first = lower_helper(_items, item);
